...
{
	//TODO:: What should happen when a texture is duplicated? Currently, when this happens,
	//the output replaces the old and the texture copied from should default to baked no longer being udpdated by teh graph instance.
	//Should this be the default functionality or should be create a new graph instance with this texture?

	Super::PostDuplicate(bDuplicateForPIE);

	if (!bDuplicateForPIE)
	{
		USubstanceTexture2D* RefTexture = nullptr;
		...
		ParentInstance = RefTexture->ParentInstance;
		...
		Substance::Helpers::RenderAsync(ParentInstance->Instance);
