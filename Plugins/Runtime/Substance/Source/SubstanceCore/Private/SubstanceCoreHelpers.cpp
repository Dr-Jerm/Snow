...
void CreateTransientTextures(USubstanceGraphInstance* Graph, UObject* WorldContextObject)
{
	...
	if (!hasRoughness && GlossinessOutput && !bCreateAllOutputs)
	{
		CreateSubstanceTexture2D(GlossinessOutput, true, FString(), Outer, Graph);
	}
...
int32 UpdateInput(SubstanceAir::GraphInstance* Instance, const SubstanceAir::InputInstanceBase* InputInstance, class UObject* InValue)
{
	...
	{
		if ((*ItInDesc)->mDesc.mUid == InputInstance->mDesc.mUid)
		{
...
void ResetMaterialTexturesFromGraph(USubstanceGraphInstance* Graph, UMaterial* OwnedMaterial, TArray<MaterialParameterSet>& Materials)
{
#if WITH_EDITOR
	auto MatItr =  Materials.CreateIterator();
	for (; MatItr; ++MatItr)
	{
		auto OutItr = Graph->Instance->getOutputs().cbegin();
		for (; OutItr != Graph->Instance->getOutputs().cend(); ++OutItr)
		{
			//Check to see if this output is referenced in the current material
			if ((*MatItr).ParameterNames.Contains((*OutItr)->mDesc.mIdentifier.c_str()))
			{
		...
		{
			(*MatItr).Material->PreEditChange(NULL);
...
#define LOCTEXT_NAMESPACE "SubstanceHelpers"
	...
	for (TObjectIterator<USubstanceGraphInstance> Itr; Itr; ++Itr)
	{
		...
		for (; OutItr != Graph->Instance->getOutputs().cend(); ++OutItr)
		{
			SubstanceAir::OutputInstance* Output = (*OutItr);
			if (Output && Output->mEnabled)
			{
		...
		//Render! NOTE:: This needs to be sync for this loop to properly work!
